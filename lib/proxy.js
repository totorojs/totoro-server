...
Proxy.proxy = function(req, res) {
  ...
  if (parsedP.query.__order_id ) {
    var id = parsedP.query.__order_id
  } else if (ref) { // ref may be undefined
    var parsedRef = url.parse(ref, true)
    var id = parsedRef.query.__order_id
  }
  ...
  } else if (!parsedP.query.__order_id && (/\.html$/.test(parsedP.pathname) || /text\/html/.test(req.headers.accept))) {
    logger.debug('Redirect iframe/window.', logInfo)
    var hasQuery = p.indexOf('?') !== -1
    var redirectP = p.replace(/(#.*$)|$/, (hasQuery ? '&' : '?') + '__order_id=' + id + '$1')
    res.redirect(redirectP)

  } else {
    logger.debug('Proxy request.', logInfo)
    p = p.replace(/(\?|&)?__order_id=[^&#]+/, '')
