...
Proxy.prototype.decorate = function(p, info, cb) {
    ...
    var runner = order.parsedRunner.path
    var proj = path.join(path.dirname(runner), '..')
    ...
    if (runner === p) {
        ...
        adapt(str, order.adapter, function(err, str) {
            if (err) {
                logger.error('Adapt runner<' + p + '> of order<' + orderId + '>, result<failed>')
            ...
            logger.debug('Adapt runner<' + p + '> of order<' + orderId + '>, result<success>')
            info.body = iconv.encode(str, charset)
            cb(info)
        })
    } else if (
        ['src', 'lib', 'dist'].some(function(item) {
            return p.indexOf(path.join(proj, item)) > -1
        }) && path.extname(p) === '.js') {

        logger.debug('Insert cov code into file<' + p + '> of order<' + orderId + '>')
        charset = order.charset || detectCharset(buffer)
        str = iconv.decode(buffer, charset)
