'use strict';

var inherits = require('util').inherits
var EventEmitter = require('events').EventEmitter
var detector = require('detector')
var logger = require('./logger')
var uaAugment = require('./ua-augment')


module.exports = Labor


function Labor(socket, data) {
  var that = this
  this.id = socket.id
  this.socket = socket

  var ua = detector.parse(data.ua)
  uaAugment.mappingOsVersion(ua)
  ua.group = uaAugment.getGroup(ua)
  ua.toString = function() {
    return ua.browser.name + ' ' + ua.browser.fullVersion + ' / ' +
         ua.os.name + ' ' + ua.os.fullVersion
  }
  this.ua = ua
  this.driverId = data.driverId

  this.orders = {}
  this.isBusy = false

  this.ping()

  socket.on('report', function(data) {
    that.report(data)
  })

  socket.on('disconnect', function() {
    that.destroy()
  })

  logger.info('New labor', {
    ua: this.ua.toString(),
    laborId: this.id,
    driverId: this.driverId
  })
}

inherits(Labor, EventEmitter)


Labor.prototype.ping = function() {
  var that = this
  var count = 0
  var ping = 0

  this.socket.on('ping', function(data) {
    ping++
  })

  this.pingTimer = setInterval(function() {
    // console.log('Check ping:', that.ua.browser.name, ping, count)
    if (ping) {
      count -= ping
      if (count < 0) count = 0
      ping = 0
    } else {
      count++
    }

    if (count >= 6) {
      that.emitRetire()
    }
  }, 10000)
}

Labor.prototype.emitRetire = function() {
  if (!this.pingTimer) return

  logger.info('Labor retires.', {
    ua: this.ua.toString(),
    laborId: this.id,
    driverId: this.driverId
  })

  clearInterval(this.pingTimer)
  this.pingTimer = undefined
  this.emit('retire')
}

Labor.prototype.add = function(order) {
  var orderId = order.id
  this.orders[orderId] = order
  this.socket.emit('add', {
    orderId: orderId,
    href: order.parsedRunner.href,
    uaGroup: this.ua.group,
    verbose: order.verbose
  })
  logger.debug('Labor adds order', {
    laborId: this.id,
    orderId: orderId
  })

  // TODO improve the boundary values of isBusy
  if (Object.keys(this.orders).length === 5) {
    this.isBusy = true
    this.emit('busy')
    logger.debug('Labor is busy', {laborId: this.id})
  }
}

Labor.prototype.remove = function(orderId, isTimeout) {
  var socket = this.socket

  /*
   * NOTE
   *
   * this.remove() can be triggered by several conditions
   *   - Browser send a 'end' report
   *   - Order destroy (finish or timeout)
   * so, to avoid loop caused by custom event, need to check it first
   */
  if (orderId in this.orders) {
    delete this.orders[orderId]
    socket.emit('remove', orderId)
    logger.debug('Labor removes order', {
      laborId: this.id,
      orderId: orderId
    })

    if (this.isBusy) {
      this.isBusy = false
      this.emit('free')
      logger.debug('Labor is free', {laborId: this.id})
    }
  }

  if (isTimeout) this.emitRetire()
}

Labor.prototype.report = function(data) {
  var that = this
  var orders = this.orders
  data.forEach(function(item) {
    var orderId = item.orderId
    var order = orders[orderId]
    /*
     * NOTE
     *
     * when client disconnect, Labor will remove this unfinished Order
     * but browser not know this sync
     * it may send report before it recieves the instruction
     * so, need to check it first
     */
    if (order) {
      item.laborId = that.id
      ;delete item.orderId
      order.report(item)

      if (item.action === 'end') {
        that.remove(orderId)
      }
    }
  })
}

Labor.prototype.destroy = function() {
  var that = this
  var laborId = this.id

  Object.keys(this.orders).forEach(function(orderId) {
    that.orders[orderId].remove(laborId)
  })

  this.emitRetire()

  logger.info('Labor destroys', {
    ua: this.ua.toString(),
    laborId: this.id,
    driverId: this.driverId
  })
  this.emit('destroy')

  this.removeAllListeners()
}
