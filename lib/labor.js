...
function Labor(socket, data) {
    ...
    this.id = socket.id
    this.socket = socket
    this.ua = useragent.lookup(data.ua)
    ...
    logger.debug('new labor: ' + this.id + ' [' + this.ua.toString() + ']')
}
...
Labor.prototype.report = function(data) {
    ...
    data.forEach(function(item) {
        ...
        if (order) {
            ...
            if (item.action === 'end') {
                that.remove(orderId)
                logger.debug('labor: ' + that.id + ' finish order: ' + orderId)
            }
        }
    })
}
...
Labor.prototype.destroy = function() {
    var that = this
    var laborId = this.id
    logger.debug('labor destroy: ' + this.id)
