...
OrderResult.prototype = {
    initOrder: function(line) {
        ...
        this.endReg = new RegExp('Order<' + orderId + '> destroy[,.]?(?: total time<([^>]+)>)?$')
    ...
    parse: function(line) {
        ...
        if (this.laborReg.test(line)) {
            ...
            // end parse
            var endMatch = line.match(this.endReg)
            if (endMatch[1]) {
                this.totalTime = endMatch[1]
            }
            this.endParse()
        }
...
module.exports.BrowserSchema = new Schema({
    ua: {type: String},
    coverge: {type: Object},
...
module.exports.LogSchema = new Schema({
    date: {type: String},
    repo: {type: String},
    orderId: {type: String},
...
module.exports.RepoInfoSchema =  new Schema({
    repo: {type: String},
...
function MongoLog(cfg) {
    ...
    var Log = db.model('Log', module.exports.LogSchema)
    var RepoInfo = db.model('RepoInfo', module.exports.RepoInfoSchema)
    ...
    this.repoInfoUpdateQueue = async.queue(function(log, callback) {
        ...
        var RepoInfoModel = that.RepoInfoModel
...
MongoLog.prototype = {
    appendLog: function(log) {
        ...
        this.updateRepoInfo(log)

        logModel.save(function(err) {
            if (err) {
                console.warn('save log error | ' + JSON.stringify(log))
            }
        })
    },
