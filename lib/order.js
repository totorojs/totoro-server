...
function Order(socket, data) {
    ...
    logger.debug('Parsed runner<' + JSON.stringify(this.parsedRunner) + '>')
    ...
    logger.debug('Parsed browsers<' + JSON.stringify(this.parsedBrowsers) + '>')
    ...
    */}
    ...
    // in half minute, at least one labor is added, or will be forced to quit
...
Order.prototype.checkIsEndAll = function() {
    ...
    logger.debug('All labors finished order<' + this.id + '>')
...
Order.prototype.destroy = function() {
    ...
    logger.debug('Order<' + this.id + '> destroy, total time<' + duration + '>')
    this.emit('destroy')
}
