...
function Order(socket, data) {
  ...
  if (typeof data.labors === 'string') {
    data.labors = [data.labors]
  }
  this.config = data
...
Order.prototype.remove = function(laborId /*private param*/, finished) {
  logger.info('Order removes labor', {orderId: this.id, laborId: laborId, finished: !!finished})
  ...
  if (finished) {
    labor.matchedLabor = labor.instance.trait
...
Order.prototype.report = function(data) {
  ...
  switch (action) {
    ...
    case 'onerror':
      ...
      break
    case 'partial':
      if (Array.isArray(labor.info)) {
        labor.info.push(info)
      } else {
        labor.info = [info]
      }
      break
    case 'end':
      this.reports.push({
        action: 'debug',
        info: 'Labor <' + trait.tostr(labor.instance.trait) + ' > finished order.'
      })
...
Order.prototype.wrapLabors = function() {
  ...
  Object.keys(labors).forEach(function(laborId) {
    ...
    var matchedLabor = ('instance' in item) ? item.instance.trait : item.matchedLabor
    rt[traitStr] = {
      requiredLabor: parsedLabors[traitStr],
      matchedLabor: matchedLabor,
