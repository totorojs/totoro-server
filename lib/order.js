...
Order.prototype.add = function(labor, browser) {
    ...
    if (this.browsersStore[browser] > 2) {
        this.report({
            action: 'end',
            info: {
                error:{
                    message: 'Unable to allocate the browser'
                }
            },
            laborId: laborId
        })
