...
function ProxyCache(id) {
    var that = this
    var cache = this.cache = {}
    var order = this.order = manager.orders[id]
    var p = order.parsedRunner.path
    ...
    order.on('destroy', function() {
        delete ProxyCache._caches[id]
    })
}

var runnerReg = /\/runner\/([^/]+)(\/.*)/
ProxyCache.get = function(req, res) {
    var match = req.url.match(runnerReg)
    var id = match[1]
    var p = match[2]

    ProxyCache._getProxy(id).getData(p, req, res)
}
...
ProxyCache._getProxy = function(id) {
    var caches = ProxyCache._caches
