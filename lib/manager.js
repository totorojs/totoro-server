  ...
  autoLabors: function() {
    ...
    Object.keys(labors).forEach(function(laborId) {
      ...
      if (trait.agent.name.toLowerCase() === 'ie' ) {
        rt[trait.agent.name + '/' + trait.agent.version] = true
      } else {
...
function isMatch(order, traitStr, labor) {
  logger.debug('Try to math', {traitStr: traitStr, laborTrait: labor.trait})
  ...
  return laborTrait.every(function(item, idx) {
    ...
    if (!item2 || item.toLowerCase().match(item2.toLowerCase())) {
      return true
    }
  })
}
