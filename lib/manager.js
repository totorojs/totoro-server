...
module.exports = {
  ...
  addDriver: function(socket, data) {
    ...
    if (id in this.drivers) {
      logger.warn('Repeated driver id.')
      return
    }

    this.drivers[id] = socket
    socket.on('disconnect', function() {
      logger.debug('Manager removes driver', {driverId: id})
      delete that.drivers[id]
    })

    logger.debug('Manager adds driver', {driverId: id})
  },
  ...
  addLabor: function(socket, data) {
    ...
    labor.on('retire', function() {
      ...
      if (driverId && (driverId in that.drivers)) {
