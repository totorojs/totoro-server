...
module.exports = {
    ...
    addLabor: function(socket, data) {
        ...
        labor.on('free', function() {
            if (!labor.isTired) {
                ...
                logger.debug('Distribute() because labor is free.')
                var b = {}
                b[laborId] = labor
        ...
        logger.debug('Distribute() because labor is added.')
        var b = {}
        b[laborId] = labor
    ...
    removeLabor: function(laborId) {
        ...
        if (labor && Object.keys(labor.orders).length){
            ...
            logger.debug('Distribute() because labor is removed with unfinished orders.')
            this.distribute(o)
        }
    },
    ...
    addOrder: function(socket, data) {
        ...
        logger.debug('Distribute() because order is added.')
        var o = {}
        o[orderId] = order
        this.distribute(o)
    },
    ...
    distribute: function(waitingOrders, availableLabors) {
        var waitingOrders = waitingOrders || this.waitingOrders
        var availableLabors = availableLabors || this.availableLabors
