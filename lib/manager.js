...
module.exports = {
    ...
    removeLabor : function(laborId) {
        ...
        for(var i = 0, len = typeLabors.length; i < len; i++) {
            if (typeLabors[i].laborId === laborId) {
    ...
    addOrder : function(socket, data) {
        ...
        if(!data.browsers) {
           ...
           data.browsers = this._filterTypeBrowsers(data.browsers)
        }
    ...
    _filterTypeBrowsers: function(browsers) {
        var that = this
        var bs = []
        browsers.forEach(function(bname) {
            if (deviceType.indexOf(bname) > -1) {
                bs = bs.concat(that._getBrowsersNameByType(bname))
            } else {
                bs.push(bname)
            }
        })
    ...
    _getBrowsersNameByType: function(type) {
    ...
    list : function() {
        ...
        this._visitLabors(function(labor) {
            var ua = labor.ua
            var type = labor.type
            var rt = typeRt[type] || (typeRt[type] = {})
            var uaStr = ua.toString()
            rt[ua] = (rt[ua] || 0) + 1
