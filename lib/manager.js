...
module.exports = {
  ...
  autoLabors: function() {
    ...
    Object.keys(labors).forEach(function(laborId) {
      ...
      if (trait.browser.name.toLowerCase() === 'ie' ) {
        rt[trait.browser.name + '/' + trait.browser.version] = true
      } else {
        rt[trait.browser.name] = true
      }
    })
...
function isMatch(order, browser, labor) {
  ...
  var trait = labor.trait
  return [ trait.os.name, trait.browser.name, trait.browser.version ].every(
    function(item, idx) {
      /*
       * TODO:
       *
